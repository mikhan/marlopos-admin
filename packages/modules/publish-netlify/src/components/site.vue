    ...
    <v-card class="site-card">
      ...
      <v-card-text>
        ...
        <div style="display: flex; align-items: center; gap: 10px">
          ...
          <v-checkbox v-model="autoPublishEnabled" v-bind:disabled="autoPublishUpdating" label="" />
        </div>
        <br />

        <!-- Details of Published Deploy -->
        <div class="site-card-details" v-if="publishedDeploy">
          <div class="site-card-details-toggle"
            @click="display_published_deploy_details = !display_published_deploy_details">
            <p>
              <strong>Published Deploy:</strong>
              <template v-if="!display_published_deploy_details">&nbsp;{{ displayDate(publishedDeploy.created_at)
              }}</template>
            </p>
            <v-icon class="site-card-details-toggle-icon"
              :name="display_published_deploy_details ? 'expand_less' : 'expand_more'"></v-icon>
          </div>
          <transition-expand>
        ...
        <div class="site-card-details" v-if="latestActivity">
          <div class="site-card-details-toggle"
            @click="display_latest_activity_details = !display_latest_activity_details">
            <p>
              <strong>Last Database Update:</strong>
              <template v-if="!display_latest_activity_details">&nbsp;{{ displayDate(latestActivity.timestamp)
              }}</template>
            </p>
            <v-icon class="site-card-details-toggle-icon"
              :name="display_latest_activity_details ? 'expand_less' : 'expand_more'"></v-icon>
          </div>
          <transition-expand>
